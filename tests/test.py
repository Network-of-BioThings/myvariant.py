...
sys.stderr.write(
    '"myvariant {0}" loaded from "{1}"\n'.format(
        myvariant.__version__, myvariant.__file__
    )
)
...
class TestMyVariantPy(unittest.TestCase):
    ...
    def test_format_hgvs(self):
        self.assertEqual(myvariant.format_hgvs("1", 35366, "C", "T"), "chr1:g.35366C>T")
        self.assertEqual(
            myvariant.format_hgvs("chr2", 17142, "G", "GA"), "chr2:g.17142_17143insA"
        )
        self.assertEqual(
            myvariant.format_hgvs("1", 10019, "TA", "T"), "chr1:g.10020del"
        )
        self.assertEqual(
            myvariant.format_hgvs("MT", 8270, "CACCCCCTCT", "C"), "chrMT:g.8271_8279del"
        )
        self.assertEqual(
            myvariant.format_hgvs("7", 15903, "G", "GC"), "chr7:g.15903_15904insC"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            myvariant.format_hgvs("20", 1234567, "GTC", "GTCT"),
    ...
    def test_querymany_with_scopes(self):
        qres = self.mv.querymany(
            ["rs58991260", "rs2500"], scopes="dbsnp.rsid", verbose=False
        )
        self.assertEqual(len(qres), 2)

        qres = self.mv.querymany(
            ["RCV000083620", "RCV000083611", "RCV000083584"],
    ...
    def test_querymany_fields(self):
        ...
        qres1 = self.mv.querymany(
            ids,
            scopes="cosmic.cosmic_id",
            fields=["cosmic.tumor_site", "cosmic.cosmic_id"],
            verbose=False,
        )
    ...
    def test_querymany_notfound(self):
        qres = self.mv.querymany(
            ["rs58991260", "rs2500", "NA_TEST"], scopes="dbsnp.rsid", verbose=False
        )
        self.assertEqual(len(qres), 3)
